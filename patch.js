 ...
 * the CLI produces no output and hangs indefinitely because:
 * 1. patchStdio() redirects stdout/stderr to coreEvents instead of real output
 * 2. stdin is consumed by readStdin() before ACP can use it for JSON-RPC
 *
 * Fix: Check for --experimental-acp at the very start of main() BEFORE patchStdio()
 * and skip directly to runZedIntegration().
 */

const fs = require('fs');
const path = require('path');

// Code to insert at the very beginning of main(), before patchStdio()
const PATCH_CODE = `
...
// The marker to find where to insert the patch - right after main() declaration
const MARKER = `export async function main() {`;
...
function applyPatch(filePath) {
    ...
    // Check if already patched with new patch
    if (content.includes(PATCH_MARKER) && content.includes('process.argv.includes')) {
        console.log('File is already patched with the correct fix. Skipping.');
        return true;
    }

    // Remove old patch if present (the one that was too late)
    if (content.includes(PATCH_MARKER)) {
        console.log('Removing old (incorrect) patch...');
        // Remove old patch code
        const oldPatchStart = content.indexOf('    // [PATCHED] Early exit for ACP mode');
        if (oldPatchStart !== -1) {
            // Find the end of the old patch (next line starting with "    // hop into sandbox")
            const oldPatchEnd = content.indexOf('    // hop into sandbox', oldPatchStart);
            if (oldPatchEnd !== -1) {
                content = content.substring(0, oldPatchStart) + content.substring(oldPatchEnd);
            }
        }
    }
    ...
    // Insert patch right after "export async function main() {"
    const insertPoint = markerIndex + MARKER.length;
    const before = content.substring(0, insertPoint);
    const after = content.substring(insertPoint);
    const patchedContent = before + PATCH_CODE + after;

    // Backup original (from the .backup file if it exists, otherwise current)
